...
- 제품 정의: 문서·코드·설계·리뷰 안건·테스트 과정과 결과·업무 보고 자료를 이해하기 쉽고 시각적으로 완성도 높은 독립형 HTML 업무 산출물로 변환하는 도구
- 영문 태그라인: **Turn complex work into clear form.**
- 한국어 설명: **복잡한 업무를 명확한 형태로.**
- CLI 이름: `forma`
- Agent Skill 이름: `forma`
- 기본 산출물: 오프라인에서 열리는 자체 완결형 `index.html`

`Forma`는 이 문서(복제본) 기준으로 확정된 제품명이다. 공개 배포 직전에는 그래도 GitHub 저장소명, npm 패키지명, 도메인, 상표 충돌을 조사하고 결과를 사용자에게 보고한다. 충돌 가능성이 높으면 이름만 바꾸되 제품 철학과 내부 구조는 유지한다.
...
Forma는 “예쁜 HTML 생성기”가 아니다. 다음 원칙을 코드와 디자인에 반영한다.
...
`FormaSpec`을 Zod와 JSON Schema로 정의한다. canonical 파일은 `forma.spec.json`으로 한다.
...
선택 후 `DESIGN.md`를 Forma 고유 디자인 시스템의 단일 원천으로 작성한다.
...
```bash
forma init
forma render ./forma.spec.json --out ./forma-output
forma validate ./forma.spec.json
forma preview ./forma-output/index.html
forma qa ./forma-output/index.html
...
```bash
forma generate --mode explain ./docs/architecture.md
forma generate --mode review ./artifacts/change.diff
forma generate --mode test ./test-results/
forma generate --mode report ./notes.md --audience manager
```
...
- Codex 프로젝트: `.agents/skills/forma/`
- Claude Code 프로젝트: `.claude/skills/forma/`
...
```text
Codex: $forma 현재 설계 문서를 이해하기 쉬운 HTML로 시각화해줘
Codex: $forma 현재 diff를 내일 코드리뷰 회의용으로 만들어줘
Claude Code: /forma 이 테스트 결과 폴더를 시각 리포트로 만들어줘
Claude Code: /forma 이 분석 메모를 팀장 보고용으로 변환해줘
```
...
16. Codex에서 `$forma`로 사용할 수 있다.
17. Claude Code에서 `/forma`로 사용할 수 있다.
